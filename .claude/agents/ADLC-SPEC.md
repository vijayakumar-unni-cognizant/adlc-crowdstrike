...
### Co-orchestration relay — parent-session contract (applies to every CLI)

`aem-program-agent` never invokes a specialist itself. It writes a dispatch packet to `runs/{run-id}/dispatch/` and ends its turn with one or more fenced `DISPATCH-REQUEST` blocks (format in `aem-program-agent.md` §"Co-orchestration dispatch mode"). **Whatever session invoked `aem-program-agent` — the top-level Claude Code / Copilot CLI / Codex CLI conversation — is that "parent session"** and MUST run this relay loop, or no specialist ever executes and the run silently dead-ends after the first dispatch:

1. Invoke `aem-program-agent`.
2. If the response ends with `DISPATCH-REQUEST` block(s): for each, read `packet:` for the specialist's input and invoke the named `agent:` with it. Invoke every block from the same turn in parallel (fan-out).
3. Once each invoked specialist has written its `expected-handoff:` file, re-invoke `aem-program-agent` with `RESUME run-id=<run-id> stage=<NN> handoff=<expected-handoff>` (batch multiple handoffs from a parallel fan-out into one resume call).
4. Repeat 2–3 until a turn has **no** `DISPATCH-REQUEST` block — that means a human checkpoint (§ "Human-in-the-loop checkpoints" above) or the final handoff (below) was reached.

A single `DISPATCH-REQUEST` is never the end of a run. Never answer on a specialist's behalf inline instead of invoking it.
...
| Model | Default for | Input ($/M) | Output ($/M) | Cache read ($/M) | Cache write 5m ($/M) | Cache write 1h ($/M) |
|---|---|---|---|---|---|---|
| `claude-opus-4-7` | Strategist | $5.00 | $25.00 | $0.50 | $6.25 | $10.00 |
| `claude-sonnet-4-6` | Program Agent, Designforge, Blockwright, Configsmith, Bridgesmith, Composer, Auditron, Sentinel | $3.00 | $15.00 | $0.30 | $3.75 | $6.00 |
| `claude-haiku-4-5` | Pilot | $1.00 | $5.00 | $0.10 | $1.25 | $2.00 |

> **Cost formula:** `cost_usd = (input_tokens × input_rate + output_tokens × output_rate + cache_read_tokens × cache_read_rate + cache_write_5m_tokens × cache_write_5m_rate + cache_write_1h_tokens × cache_write_1h_rate) / 1_000_000`
>
> Cache reads substantially cut cost in long runs — the 5-minute prompt-cache TTL means specialists called within 300 s of each other save 40–60% on input costs. The 1-hour cache write tier (2× base input price) is available for longer-lived context but is not the default in ADLC runs; most ADLC cache writes use the 5-minute tier.
>
> **Tokenizer note:** `claude-opus-4-7` and later Opus models use a newer tokenizer that produces approximately 30% more tokens for the same text compared to `claude-sonnet-4-6` and earlier models. The §10.2 Strategist token ranges reflect actual token counts (post-tokenizer); budget comparisons between Strategist and Sonnet specialists must account for this difference.
...
| Strategist | opus-4-7 | `init`, `ensure-agents-md`, `best-practices` | 12K – 28K | 2 – 5 min | $0.05 – $0.16 |
...
| Pilot | haiku-4-5 | `aem-rde` only (Local + RDE scope only; no operations track) | 8K – 20K | 1 – 4 min | $0.01 – $0.03 |

**Estimated per-run totals:**

| Run scope | Token range | Duration range | Estimated cost |
|---|---|---|---|
| Moderate scope (1 feature, ~5 components, 1 template, no migration) | 150K – 550K | 25 – 70 min | $1.90 – $7.20 |
| Wide scope (URL rebuild + migration + multi-component + RDE deploy + content seeding) | 550K – 1.3M | 70 – 140 min | $7.20 – $15.70 |
...
**These reports are a BLOCKING session-close gate, not a nice-to-have.** No run — pass, fail, or paused — is reportable-complete until all five files exist under `runs/{run-id}/reports/`: `tokens.json`, `tokens-report.html`, `skills.md`, `final-report.md`, `demo-script.md` (see `aem-program-agent.md § P13`). They are written **before** the human-handoff packet, and every path is referenced from it. Two standing rules:
...
For every actual run the Program Agent writes three files:

**`reports/tokens.json`** — machine-readable, full token + cost + duration ledger. Every agent entry uses the same full field set; `null` plus a `measurement_gap` note when a value is genuinely unavailable:
...
```json
{
  ...
  "generated_at": "2026-06-29T19:20:00Z",
  "totals": {
    "input_tokens": 0,
    "output_tokens": 0,
    "cache_read_tokens": 0,
  ...
  "pricing_ref": {
    "claude-opus-4-7":   { "input_per_m": 5.00,  "output_per_m": 25.00, "cache_read_per_m": 0.50, "cache_write_5m_per_m": 6.25,  "cache_write_1h_per_m": 10.00 },
    "claude-sonnet-4-6": { "input_per_m": 3.00,  "output_per_m": 15.00, "cache_read_per_m": 0.30, "cache_write_5m_per_m": 3.75,  "cache_write_1h_per_m": 6.00  },
    "claude-haiku-4-5":  { "input_per_m": 1.00,  "output_per_m": 5.00,  "cache_read_per_m": 0.10, "cache_write_5m_per_m": 1.25,  "cache_write_1h_per_m": 2.00  }
  },
  "by_agent": {
    "program-agent": {
      ...
      "stage": "orchestration",
      "status": "pass",
      "skill_invocations": ["ensure-agents-md"],
      "input_tokens": 0,
      "output_tokens": 0,
      "cache_read_tokens": 0,
      "cache_write_5m_tokens": 0,
    ...
    "strategist": {
      ...
      "stage": "plan",
      "status": "pass",
      "skill_invocations": ["init", "ensure-agents-md", "best-practices"],
      "input_tokens": 0,
      "output_tokens": 0,
      "cache_read_tokens": 0,
      "cache_write_5m_tokens": 0,
    ...
    "blockwright": {
      ...
      "stage": "implement",
      "status": "pass",
      "skill_invocations": ["create-component", "create-editable-template", "best-practices"],
      "input_tokens": 0,
      "output_tokens": 0,
      "cache_read_tokens": 0,
      "cache_write_5m_tokens": 0,
    ...
    "configsmith": {
      ...
      "stage": "implement",
      "status": "pass",
      "skill_invocations": ["repoinit", "security-review", "dispatcher"],
      "input_tokens": 0,
      "output_tokens": 0,
      "cache_read_tokens": 0,
      "cache_write_5m_tokens": 0,
    ...
    "bridgesmith": {
      ...
      "stage": "integrate",
      "status": "pass",
      "skill_invocations": ["best-practices", "content-distribution"],
      "input_tokens": 0,
      "output_tokens": 0,
      "cache_read_tokens": 0,
      "cache_write_5m_tokens": 0,
    ...
    "composer": {
      ...
      "stage": "integrate",
      "status": "pass",
      "skill_invocations": ["create-content-fragment-graphql"],
      "input_tokens": 0,
      "output_tokens": 0,
      "cache_read_tokens": 0,
      "cache_write_5m_tokens": 0,
...
**`reports/tokens-report.html`** — human-readable visual report rendered from `tokens.json` using `.claude/agents/references/tokens-report-template.html`. Generated by the Program Agent at session close immediately after writing `tokens.json`. Shows run summary stat cards, token & cost breakdown table, per-agent cost distribution bar chart, per-agent detail table (all token fields, cost, duration, skills), and the pricing snapshot used. Standalone — no network dependencies.
